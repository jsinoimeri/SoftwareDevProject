...
public class TicTacToe extends Game implements java.io.Serializable
{
	...
	protected Stack<TicTacToeBoard> undo;
	protected Stack<TicTacToeBoard> redo;
	/**
	...
	public TicTacToe(Board board)
	{
		...
		this.undo = new Stack<TicTacToeBoard>();
		this.redo = new Stack<TicTacToeBoard>();
	}
	...
	public void play()
	{
		...
		while(winner == null && !draw)
		{
			...
			if (winner == null && !draw)
			{
				...
				this.getUndo().push(new TicTacToeBoard(this.getBoard()));
	...
	protected boolean checkMove(Player player)
	{
		...
		if (move_col == HELPNEEDED)
		{
			...
			SaveAndLoad.saveGame(this.getBoard(), SAVEFILE);
			this.saveRequired = true;
			return false;
		}
		...
		else if (move_row < this.getBoard().getSize() && move_col < this.getBoard().getSize() && !this.getBoard().isSquareOccupied(move_row, move_col))
		{
	...
	private void makeMove(Player player)
	{
		...
		while (!this.checkMove(player))
		{
			...
			if (!this.helpRequired && !this.redoRequired && !this.undoRequired && !this.saveRequired && !this.loadRequired)
			    System.out.println(player.toString() + ": " + INVALIDOPTION);
	...
	public void undo() {
		...
		if(this.canUndo()){
			this.getRedo().push(new TicTacToeBoard(this.getBoard()));
	...
	public void redo() {
		...
		if(this.canRedo()){
			this.getUndo().push(new TicTacToeBoard(this.getBoard()));
			this.setBoard(this.getRedo().pop());
		}
	}
	
	public Stack<TicTacToeBoard> getUndo() {
		return undo;
	}


	public Stack<TicTacToeBoard> getRedo() {
		return redo;
	}
